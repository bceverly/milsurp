...
def _to_out(row: Manufacturer, counts: dict[str, int]) -> ManufacturerOut:
    ...
    out.item_count = counts.get(row.name, 0)
    return out
...
def create_manufacturer(
    ...
    return ManufacturerWrite(manufacturer=_to_out(row, _counts(session)), listings_changed=changed)
...
def update_manufacturer(
    ...
    service.invalidate()
    changed = service.reprocess(session, [*was, *row.spellings])
